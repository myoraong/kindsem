...
export function OvertimePay({ item }: { item: CalcItem }) {
  ...
  const result = useMemo(() => {
    ...
    return calcOvertimePay({
      ...
      nightHours: Number(v.night) || 0,
      holidayHours: Number(v.holiday) || 0,
    })
  }, [v])
      ...
      <div className="space-y-5">
        ...
        {v.pay === "hourly" ? (
          <div className="space-y-2">
            <MoneyField id="hourly" label="통상시급" unit="원" value={v.hourly} onChange={(value) => set("hourly", value)} />
            <AmountChips
              options={[
        ...
        <div className="space-y-2">
          <MoneyField id="overtime" label="연장시간" unit="시간" value={v.overtime} onChange={(value) => set("overtime", value)} />
          <AmountChips
            options={[
              { label: "0시간", value: "0" },
              { label: "5시간", value: "5" },
        ...
        <div className="space-y-2">
          <MoneyField id="night" label="야간시간 (22:00–06:00)" unit="시간" value={v.night} onChange={(value) => set("night", value)} />
          <AmountChips
            options={[
              { label: "0시간", value: "0" },
              { label: "4시간", value: "4" },
        ...
        <div className="space-y-2">
          <MoneyField id="holiday" label="휴일시간" unit="시간" value={v.holiday} onChange={(value) => set("holiday", value)} />
          <AmountChips
            options={[
              { label: "0시간", value: "0" },
              { label: "8시간", value: "8" },
        ...
        <Hint>
          시간은 기본을 비워 둡니다. 해당 시간만 넣으세요. 야간은 가산만 더합니다. 같은 시간을 연장과
          휴일에 겹쳐 넣으면 통상임금이 두 번 잡힙니다.
        </Hint>
