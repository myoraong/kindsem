...
test("인페이지 광고는 본문이 짧은 안내·목록·사칙 경로에는 넣지 않는다", () => {
  ...
  assert.equal(shouldRenderAdOnPath("/calc/severance/"), true)
  assert.equal(shouldRenderAdOnPath("/realty"), false)
  assert.equal(shouldRenderAdOnPath("/realty/"), false)
  assert.equal(shouldRenderAdOnPath("/calc"), false)
  assert.equal(shouldRenderAdOnPath("/calc/"), false)
  assert.equal(shouldRenderAdOnPath("/calc/quick"), false)
  assert.equal(shouldRenderAdOnPath("/calc/quick/"), false)
  assert.equal(shouldRenderAdOnPath("/how"), false)
  assert.equal(shouldRenderAdOnPath("/how/"), false)
  assert.equal(shouldRenderAdOnPath("/about"), false)
  assert.equal(shouldRenderAdOnPath("/about/"), false)
