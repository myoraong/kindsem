...
test("별칭으로 복비·DSR을 찾는다", () => {
  ...
  assert.equal(searchCalculators("간이세액표")[0]?.slug, "take-home")
  assert.equal(searchCalculators("명세서")[0]?.slug, "take-home")
  assert.equal(searchCalculators("3.3%")[0]?.slug, "side-job-tax")
  assert.equal(searchCalculators("연납")[0]?.slug, "car-tax")
